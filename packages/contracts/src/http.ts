...
export const MobileLoginRequestSchema = LoginRequestSchema
...
export const MobileSessionInfoSchema = z.object({ authenticated: z.literal(true), username: z.string(), expires_at: z.string().datetime() }).strict()
export const MobileLoginResponseSchema = MobileSessionInfoSchema.extend({ token: z.string().min(43).max(128) }).strict()
