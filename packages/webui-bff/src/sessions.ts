...
export class SessionStore { private sessions=new Map<string,Session>(); private file:string; constructor(private dir:string,private ttlMs:number,private idleMs:number){this.file=join(dir,'sessions.json')}
 ...
 async create(username:string){this.sessions.clear();const sid=randomBytes(32).toString('base64url');const now=Date.now();const s={hash:this.digest(sid),csrf:randomBytes(32).toString('base64url'),username,created:now,lastSeen:now};this.sessions.set(s.hash,s);await this.save();return{sid,session:s}}
